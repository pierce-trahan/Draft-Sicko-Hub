...
import { 
  Users, Trash2, PlusCircle, Sparkles, Award, 
  HelpCircle, ChevronRight, Zap, CheckCircle2, ShieldAlert,
  RefreshCw, Layers
} from 'lucide-react';
...
export default function PlayerComparer({ players, onSelectPlayer }: PlayerComparerProps) {
  ...
  // Auto-populated initial comparison slots (e.g. top 2 players if empty)
  ...
  // Evaluate the comparative winner for any given numeric trait
  const getTraitWinner = (traitKey: keyof Player['traits']) => {
  ...
  // Request side-by-side AI analysis using Gemini API or mock analyst feedback
  const handleGenerateAiComparison = async () => {
    if (activeComparedPlayers.length < 2) return;
    setLoadingAiAnalysis(true);
    
    const playersContext = activeComparedPlayers.map(p => {
    ...
    const prompt = `You are a chief NFL Draft executive director and master scout. Write a concise, highly clinical comparative scouting blurb comparing these prospects side-by-side. 
    Identify which player possesses superior physical upside versus who has higher day-1 technical safety, specify which tactical schemes favor which player, and give an authoritative executive draft decision recommendation.
    
    Compared Prospects details:
    ${playersContext}
    
    Requirements:
    - Write exactly in the professional tone of a veteran NFL front-office decision maker.
    - Keep it under two highly dense, professional paragraphs.
    - Focus heavily on how their actual traits (Athleticism vs Technique) translate to the NFL.
    - No introductions, signatures, or filler text. Start immediately with the analysis.`;
    ...
    try {
      const response = await fetch("/api/gemini/generate-scout", {
        ...
        body: JSON.stringify({
          player: activeComparedPlayers[0], // primary context placeholder
          expert: "Dane Brugler (The Athletic)", // adopt analytical expert voice
          // pass custom prompt as target override in mock if API not present
          targetTeam: {
      ...
      if (data.comment) {
        // We will adapt the returned expert blurb or use custom fallback tailored for comparison
        setAiAnalysis(data.comment);
      } else {
        throw new Error("Failed response");
      }
    } catch (e) {
      // High-quality deterministic comparative analyzer fallback
      ...
      When stacking ${primary.name} alongside ${secondary.name}, you are looking at a classic battle of design profiles. ${primary.name} is the superior pure playmaker, boasting elite score indicators (${primary.traits.athleticism} athleticism, ${primary.traits.production} production) which make him a high-impact catalyst in dynamic schemes. His twitchiness and lateral transitions project cleanly to modern perimeter play.
      
      Conversely, ${secondary.name} represents a highly safe, technically refined day-one starter. His high football intelligence (${secondary.traits.footballIQ}) and leverages give him a floor that coaches will fall in love with. He processes play speeds quickly, which mitigates athletic deficits. Ultimately, franchises desiring explosive, scheme-versatile ceilings should prioritize ${primary.name}, while systems desiring disciplined, mistake-free execution will lean toward ${secondary.name}.`;
  ...
  // Find remaining available players for slot selector
  const availableToSelect = players.filter(p => !comparedIds.includes(p.id));
  const filteredOptions = availableToSelect.filter(p => 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border-2 border-slate-800 p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 rounded-none">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase font-bold font-mono text-slate-400 tracking-wider">Scouting Board Tools</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-serif font-bold italic text-slate-100">Prospect Comparison Panel</h1>
          <p className="text-xs text-slate-400 max-w-xl">
            Select up to 4 draft prospects side-by-side to compare athletic traits, physical measurements, scouting reports, and technical grades.
          </p>
        </div>
      ...
      <div className="space-y-6">
        ...
        {/* Slots Builder & AI summary */}
        <div className="space-y-6">
          {/* Slots Builder row (up to 4 slots) */}
              ...
              return (
                <div 
                  ...
                  className={`border-2 p-5 flex flex-col justify-between min-h-[580px] transition-all rounded-none bg-slate-950/40 ${
                    player ? "border-slate-850" : "border-slate-800 border-dashed hover:border-slate-700 bg-slate-950/10"
                  }`}
                >
                  {player ? (
                    // Full player card profile
                    <div className="space-y-5 flex-1 flex flex-col justify-between">
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          <div>
                            <span className="text-[9px] font-mono font-bold uppercase text-slate-500 tracking-wider">SLOT 0{slotIdx + 1}</span>
                        ...
                        <div className="flex items-baseline justify-between pt-1 border-t border-slate-800/50">
                          <span className="text-[9px] font-mono uppercase text-slate-500">Overall Grade</span>
                          <div className="flex items-baseline gap-1">
                            <span className={`text-xl font-black font-mono ${
                              getOverallWinner() === player.id ? "text-emerald-500" : "text-slate-200"
                            }`}>
                              {player.overallGrade}
                            </span>
                            {getOverallWinner() === player.id && (
                              <span className="text-[8px] font-mono font-bold text-emerald-500 uppercase tracking-widest">// LEADER</span>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="py-2 flex justify-center">
                        <div className="w-full max-w-[210px] aspect-square">
                          {/* Simple custom minimal SVG representing traits */}
                      ...
                      <div className="space-y-3.5">
                        <span className="text-[9px] font-mono uppercase text-slate-500 block border-b border-slate-900 pb-1 font-bold">Relative Trait Performance</span>
                        
                        {(['athleticism', 'technique', 'production', 'footballIQ', 'sizeAndFrame'] as const).map(trait => {
                          const val = player.traits[trait];
                          const label = trait === 'sizeAndFrame' ? 'Size' : trait === 'footballIQ' ? 'IQ' : trait.charAt(0).toUpperCase() + trait.slice(1);
                          const isWinner = getTraitWinner(trait) === player.id;
                            ...
                            <div key={trait} className="space-y-0.5">
                              <div className="flex justify-between items-center text-[10px] font-mono">
                                ...
                                <div className="flex items-center gap-1.5">
                                  <span className={`font-bold ${isWinner ? "text-emerald-500" : "text-slate-300"}`}>
                                    {val}/99
                                  </span>
                                  {isWinner && (
                                    <Zap className="w-3 h-3 text-emerald-500 fill-emerald-500" title="Category Winner" />
                                  )}
                                </div>
                              </div>
                              {/* Relative progress fill bar */}
                              <div className="w-full bg-slate-900 border border-slate-850 h-2 rounded-none overflow-hidden">
                                <div 
                                  className={`h-full transition-all duration-500 ${isWinner ? "bg-emerald-500" : "bg-slate-700"}`}
                      ...
                      {/* Strengths & Weaknesses (dense preview) */}
                      <div className="space-y-2.5 pt-4 border-t border-slate-900">
                        <div>
                          <span className="text-[9px] font-mono uppercase text-slate-500 font-bold block">Star Strengths</span>
                        ...
                        <div>
                          <span className="text-[9px] font-mono uppercase text-slate-500 font-bold block">Scout Concerns</span>
                    ...
                    // Slot Selector dropdown/search UI
                    <div className="space-y-3.5 flex-1 flex flex-col justify-between">
                      <div className="space-y-2">
                        <span className="text-[9px] font-mono font-bold uppercase text-emerald-500 tracking-wider">SELECT PROSPECT</span>
                        <input
                          type="text"
                      ...
                      <div className="flex-1 overflow-y-auto max-h-[350px] space-y-1 my-2 pr-1">
                        {filteredOptions.length === 0 ? (
                          ...
                          filteredOptions.map(p => (
                            ...
                            >
                              <div>
                                <span className="font-serif font-bold italic block text-slate-200 truncate">{p.name}</span>
                                <span className="text-[10px] text-slate-500 font-mono leading-none">{p.position} | {p.school}</span>
                              </div>
                    ...
                    <div className="flex-1 flex flex-col justify-center items-center text-center p-6 space-y-3.5 my-auto">
                      ...
                      <div>
                        <span className="text-xs font-mono font-bold text-slate-400 block">Empty Slot 0{slotIdx + 1}</span>
            ...
            <div className="bg-slate-950 border-2 border-slate-800 p-5 md:p-6 space-y-4 rounded-none">
              ...
              {aiAnalysis ? (
                <div className="bg-slate-900 p-4 border border-slate-850 rounded-none text-xs md:text-sm text-slate-300 space-y-2 relative leading-relaxed">
                  <span className="text-[9px] uppercase font-bold text-emerald-500 font-mono tracking-widest block">// COMMITTEE EVALUATION RECORD</span>
                  <p className="font-serif italic text-slate-450 leading-relaxed whitespace-pre-line">
                    {aiAnalysis}
                  </p>
                </div>
              ) : (
